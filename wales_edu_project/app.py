...
def render_page_switcher(page: str) -> None:
    ...
    st.markdown(
        ...
        .result-loader-map{
          ...
          animation:loader-map-breathe 1.8s ease-in-out infinite;
        }
        ...
        .result-loader-dots i{width:9px;height:9px;border-radius:50%;
          background:#e7b5a4;animation:loader-dot 1.2s linear infinite}
        ...
        @keyframes loader-dot{
          0%,28%,100%{background:#e7b5a4;opacity:.48;box-shadow:none}
          42%,62%{background:#d9514f;opacity:1;
            box-shadow:0 0 0 4px rgba(217,81,79,.10)}
        }
        @keyframes loader-map-breathe{
          0%,100%{filter:drop-shadow(0 18px 25px rgba(117,58,43,.17))}
          50%{filter:drop-shadow(0 18px 31px rgba(117,58,43,.27))}
        }
...
def branded_loading_overlay(message: str) -> Any:
    ...
    if logo_path.exists():
        logo_b64 = base64.b64encode(logo_path.read_bytes()).decode("ascii")
        logo_css = (
            ".result-loader-map{background-image:url('data:image/png;base64,"
            + logo_b64 + "')!important;}"
        )
    overlay_css = """
    ...
    slot.markdown(
        ...
        "<div class='result-loader-note'>This may take a moment while the "
        "Welsh geographic data is brought together.</div>"
...
def dismiss_loading_when_ready(
    *, require_map: bool, startup: bool = False
) -> None:
    ...
    components.html(
        ...
        (() => {{
          const doc = window.parent.document;
          const readySelector = {json.dumps(target)};
          const started = Date.now();
          const timer = setInterval(() => {{
            const ready = doc.querySelector(readySelector);
            const painted = ready && ready.getBoundingClientRect().width > 100
              && ready.getBoundingClientRect().height > 80;
            if (painted) {{
...
def _canonical_spatial_type(value: str | None) -> str:
    """Use the same public type names as the guided SCQ engine."""
    text = str(value or "")
    return "CommunityWard" if text == "Ward" else text
...
def _map_requested_result(text: str) -> Tuple[str, str | None]:
    ...
    prefix = re.split(
        ...
        r"adjacent|neighbours?|neighbors?|neighbouring|neighboring)\b",
        low,
        maxsplit=1,
    )[0]
...
def _map_explicit_relation(text: str) -> str:
    ...
    if (
        ...
        return "graph_near"
    if re.search(
        r"\b(?:touch|touches|touching|adjacent|neighbours?|neighbors?|"
        r"neighbouring|neighboring)\b",
        low,
    ):
        return "touches"
    return "direct"
...
def _map_admin_hint(text: str) -> Dict[str, str] | None:
    ...
    if not unit_type:
        return None
    unit_type = _canonical_spatial_type(unit_type)
    ...
    words = "|".join(
        re.escape(k) for k, v in _ADMIN_TYPE_WORDS.items()
        if _canonical_spatial_type(v) == unit_type
    )
    match = re.search(
    ...
    if not match:
        return {"anchor_name": "", "anchor_type": unit_type,
                "relation": relation,
                "target_type": _canonical_spatial_type(requested_type) or unit_type,
...
def _map_spatial_hint(text: str) -> Dict[str, str] | None:
    """Read one explicit Welsh spatial question, including an LSOA anchor.

    The same compact intent is consumed by SCQ natural search and Map
    Explorer.  The parser only identifies words and codes; Neo4j remains
    authoritative for entity resolution and relationship availability.
    """
    ...
    relation_patterns = {
        "not_touches": (
            r"\b(?:(?:does|do)\s+not\s+(?:touch|border)|not\s+touch(?:ing)?|"
            r"non[- ]adjacent|not\s+adjacent)\b"
        ),
        ...
        "direct": r"\b(?:in|inside|within)\b",
    }
    relation_match = re.search(
        relation_patterns.get(relation, relation_patterns["direct"]),
        raw,
        re.IGNORECASE,
    )
    if not relation_match:
        return _map_admin_hint(raw)
    tail = raw[relation_match.end():].strip(" ?.,")
    tail = re.sub(r"^(?:to|with|of|the)\s+", "", tail, flags=re.I)

    def parse_anchor(fragment: str) -> Tuple[str, str] | None:
        piece = str(fragment or "").strip(" ?.,")
        piece = re.sub(r"^(?:to|with|of|the)\s+", "", piece, flags=re.I)
        code = re.search(r"\bW\d{8}\b", piece, re.IGNORECASE)
        if code:
            return code.group(0).upper(), "LSOA"
        matches: List[Tuple[int, int, str]] = []
        for phrase, raw_type in _ADMIN_TYPE_WORDS.items():
            found = list(re.finditer(rf"\b{re.escape(phrase)}\b", piece, re.I))
            for item in found:
                matches.append((item.start(), item.end(), raw_type))
        if not matches:
            return None
        # The anchor's type is the final explicit geography in its fragment;
        # longest wording wins ties ("community ward" before "community").
        start, end, raw_type = sorted(
            matches, key=lambda m: (m[1], m[1] - m[0]), reverse=True
        )[0]
        name = piece[:start].strip(" ,")
        name = re.sub(r"^(?:to|with|of|the)\s+", "", name, flags=re.I)
        return (name, _canonical_spatial_type(raw_type)) if name else None

    first_fragment = tail
    second_fragment = ""
    if relation == "between":
        pair = re.split(r"\s+and\s+", tail, maxsplit=1, flags=re.I)
        if len(pair) != 2:
            return None
        first_fragment, second_fragment = pair
    ...
    if relation == "between":
        second = parse_anchor(second_fragment)
        if not second:
            return None
        hint["second_name"] = second[0]
        hint["second_type"] = second[1]
    return hint
...
def resolve_map_admin_scope(
    ...
    unit_type = _canonical_spatial_type(scope.get("anchor_type"))
    relation = str(scope.get("relation") or "direct")
    target_type = _canonical_spatial_type(scope.get("target_type") or unit_type)
    result_kind = str(scope.get("result_kind") or "Schools")
    spatial_only = bool(scope.get("spatial_only"))
    if not name or (unit_type not in _MAP_ADMIN_TYPES and unit_type != "LSOA"):
    ...
    if unit_type != "LSOA" and result_kind == "LSOA" and relation in {
    ...
    if unit_type != "LSOA" and result_kind == "LSOA" and relation == "direct":
        relation = "intersects"
    if (
        unit_type != "LSOA"
        and result_kind == "AdminUnit"
    ...
    if unit_type == "LSOA":
        ...
        MATCH (a:LSOA)
        WHERE toUpper(coalesce(a.code, '')) = toUpper($anchor_name)
           OR toLower(coalesce(a.name, '')) = toLower($anchor_name)
        RETURN a.code AS uri, coalesce(a.name, a.code) AS name,
               'LSOA' AS unit_type, a.wkt AS wkt
        ORDER BY a.code
        LIMIT 3
        """, {"anchor_name": name})
    else:
        anchor_kind = guided_admin_kind_expr("a")
        wales_anchor = guided_wales_admin_predicate("a")
        anchors = run_cypher(cfg, f"""
        MATCH (a:AdminUnit)
    ...
    if anchors.empty:
        if unit_type == "LSOA":
            return None, [f"No exact Welsh LSOA named or coded {name} was found."]
        alternative_kind = guided_admin_kind_expr("a")
        alternative_wales = guided_wales_admin_predicate("a")
        alternatives = run_cypher(cfg, f"""
        MATCH (a:AdminUnit)
        WHERE {alternative_wales}
          AND (
        ...
        RETURN DISTINCT {alternative_kind} AS unit_type ORDER BY unit_type
        """, {"anchor_name": name})
    ...
    if relation == "between":
        ...
        second_type = _canonical_spatial_type(scope.get("second_type"))
        if not second_name or second_type != unit_type or target_type != unit_type:
            return None, [
                "BETWEEN is evaluated within one geographic layer. Name two "
                "different places of the same type and request that same type."
            ]
        if unit_type == "LSOA":
            second_rows = run_cypher(cfg, """
            MATCH (b:LSOA)
            WHERE toUpper(coalesce(b.code, '')) = toUpper($name)
               OR toLower(coalesce(b.name, '')) = toLower($name)
            RETURN b.code AS uri, coalesce(b.name,b.code) AS name,
                   'LSOA' AS unit_type, b.wkt AS wkt
            ORDER BY b.code LIMIT 3
            """, {"name": second_name})
        else:
            second_kind = guided_admin_kind_expr("b")
            second_wales = guided_wales_admin_predicate("b")
            second_rows = run_cypher(cfg, f"""
            MATCH (b:AdminUnit)
            WHERE {second_kind} = $kind AND {second_wales}
              AND (
                toLower(coalesce(b.name,'')) = toLower($name)
                OR toLower(coalesce(b.name,'')) STARTS WITH toLower($name) + ' - '
                OR toLower(coalesce(b.name,'')) ENDS WITH ' - ' + toLower($name)
              )
            RETURN b.uri AS uri, b.name AS name, {second_kind} AS unit_type,
                   b.wkt AS wkt
            ORDER BY CASE WHEN toLower(coalesce(b.name,'')) = toLower($name)
                          THEN 0 ELSE 1 END, b.uri
            LIMIT 3
            """, {"kind": second_type, "name": second_name})
        if len(second_rows) != 1:
            return None, [
                f"The second {second_type} named {second_name} could not be "
                "resolved exactly."
            ]
        ...
        if rows.empty:
            return None, [
                f"No same-layer TOUCHES path was found between {name} and "
                f"{second_name}; therefore no area lies between them under "
                "the project's graph definition."
            ]
        lsoa_codes = sorted(set(
            rows.get("lsoa_code", pd.Series(dtype=str)).dropna().astype(str)
        ))
        units = rows.copy()
        if unit_type != "LSOA":
            unit_ids = units["unit_uri"].dropna().astype(str).tolist()
            coverage = run_cypher(cfg, """
            MATCH (u:AdminUnit)-[:INTERSECTS]->(l:LSOA)
            WHERE u.uri IN $unit_ids
            RETURN u.uri AS unit_uri, l.code AS lsoa_code,
                   u.name AS unit_name, u.wkt AS unit_wkt
            """, {"unit_ids": unit_ids})
            if not coverage.empty:
                units = units.merge(coverage, on="unit_uri", how="left",
                                    suffixes=("", "_coverage"))
                lsoa_codes = sorted(set(
                    units.get("lsoa_code", pd.Series(dtype=str))
                    .dropna().astype(str)
        ...
        return {
            "anchor": anchors.iloc[0].to_dict(),
            "second_anchor": second_rows.iloc[0].to_dict(),
            "relation": "between", "target_type": target_type,
            "result_kind": result_kind, "spatial_only": spatial_only,
            "provenance": "Graph-derived",
            "disjointness_applied": False,
            "lsoa_codes": lsoa_codes, "units": units,
        }, []

    # LSOA questions use exactly the same capability table and query builder
    # as the guided SCQ controls.  This is the common engine that was missing
    # when natural wording was routed through the admin-only map resolver.
    if unit_type == "LSOA":
        guided_relation = {
            "graph_near": "near",
            "not_touches": "not_touches",
        }.get(relation, relation)
        if guided_relation == "direct":
            guided_relation = "intersects" if target_type != "LSOA" else ""
        if guided_relation in {"inside", "contains"}:
            return None, [
                f"{guided_relation.upper()} is not represented between LSOAs."
            ]
        cfg_key = (cfg["uri"], cfg["user"], cfg["password"], cfg["database"])
        capabilities = guided_capabilities(cfg_key, "LSOA", anchor_uri)
        available = capabilities[
            (capabilities["relation"] == guided_relation)
            & (capabilities["result_type"] == target_type)
        ] if not capabilities.empty and guided_relation else pd.DataFrame()
        if available.empty:
            return None, [
                f"No {guided_relation.replace('_', ' ').upper()} result from "
                f"{anchor_uri} to {target_type} is available in the graph."
            ]
        query = guided_result_query("LSOA", guided_relation, target_type)
        rows = run_cypher(
            cfg, query,
            {"anchor": anchor_uri, "result_type": target_type},
        )
        lsoa_codes = sorted(set(
            rows.get("lsoa_code", pd.Series(dtype=str)).dropna().astype(str)
        ))
        units = rows.copy()
        if "unit_uri" in units.columns and not units.empty:
            units = units.rename(columns={"unit_type": "unit_type"})
            unit_ids = units["unit_uri"].dropna().astype(str).tolist()
            if unit_ids:
                coverage = run_cypher(cfg, """
                MATCH (u:AdminUnit)-[:INTERSECTS]->(l:LSOA)
        ...
        return {
            ...
            "provenance": (
                "Graph-derived" if guided_relation == "near"
                else "Represented graph relation"
            ),
            "disjointness_applied": guided_relation == "near",
    ...
    if spatial_only:
        guided_relation = {
            "graph_near": "near",
            # In a question such as "Which communities are inside Cardiff?",
            # Cardiff is the anchor and the returned communities are the
            # children it contains.  The guided engine names that direction
            # from the anchor's perspective.
            "inside": "contains",
            "contains": "within",
        }.get(relation, relation)
        cfg_key = (cfg["uri"], cfg["user"], cfg["password"], cfg["database"])
        capabilities = guided_capabilities(cfg_key, unit_type, anchor_uri)
        available = capabilities[
            (capabilities["relation"] == guided_relation)
            & (capabilities["result_type"] == target_type)
        ] if not capabilities.empty else pd.DataFrame()
        if available.empty:
            return None, [
                f"No {guided_relation.replace('_', ' ').upper()} result from "
                f"{name} ({unit_type}) to {target_type} is available in "
                "the graph."
            ]
        query = guided_result_query(unit_type, guided_relation, target_type)
        rows = run_cypher(
            cfg, query,
            {"anchor": anchor_uri, "result_type": target_type},
        )
        ...
        units = rows.copy()
        if target_type != "LSOA" and "unit_uri" in units.columns:
            unit_ids = units["unit_uri"].dropna().astype(str).tolist()
            if unit_ids:
                coverage = run_cypher(cfg, """
                MATCH (u:AdminUnit)-[:INTERSECTS]->(l:LSOA)
        ...
        if rows.empty:
            return None, [
                f"No {guided_relation.replace('_', ' ')} result was returned "
                f"for {name}."
            ]
        return {
            "anchor": anchors.iloc[0].to_dict(),
...
def llm_parse_map_question(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        system = (
            ...
            '"anchor_type": "LSOA" | "Community" | "Ward" | "CommunityWard" | '
            '"CivilParishorCommunity" | "UnitaryAuthority" | '
            '"EuropeanRegion"\n'
            '"relation": "direct" | "touches" | "not_touches" | '
            '"graph_near" | "intersects" | "inside" | "contains" | "between"\n'
            '"result_kind": "Schools" | "LSOA" | "AdminUnit"\n'
            '"target_type": one of the same geographic types\n'
            '"second_name": second exact place name for BETWEEN\n'
            '"second_type": second geographic type for BETWEEN\n'
    ...
    anchor_type = _canonical_spatial_type(data.get("anchor_type"))
    ...
    local_hint = _map_spatial_hint(text)
    if local_hint:
        ...
        relation = "touches"
    target_type = _canonical_spatial_type(
    ...
    valid_spatial_types = valid_admin_types | {"LSOA"}
    valid_relations = {
    ...
    for raw_scope in (data.get("admin_scopes") or []):
        ...
        if (
            ...
            parsed_scope = {
                ...
                "target_type": (
                    scope_target_type
                    if scope_target_type in valid_spatial_types
                    else scope_anchor_type
                ),
                "result_kind": out["result_kind"],
                "spatial_only": out["spatial_only"],
            }
            if scope_relation == "between":
                parsed_scope["second_name"] = str(
                    raw_scope.get("second_name") or ""
                ).strip()
                parsed_scope["second_type"] = _canonical_spatial_type(
                    raw_scope.get("second_type") or scope_anchor_type
                )
            out["admin_scopes"].append(parsed_scope)

    if anchor_type in valid_spatial_types:
        out["admin_scope"] = {
            "anchor_name": anchor_name,
        ...
        if relation == "between":
            out["admin_scope"]["second_name"] = str(
                (local_hint or {}).get("second_name")
                or data.get("second_name") or ""
            ).strip()
            out["admin_scope"]["second_type"] = _canonical_spatial_type(
                (local_hint or {}).get("second_type")
                or data.get("second_type") or anchor_type
            )
