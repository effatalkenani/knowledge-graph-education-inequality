...
def llm_parse_map_question(text: str) -> Dict[str, Any]:
    ...
    if admin_unit_type and spatial_relation and spatial_target:
        ...
        if spatial_relation == "near":
            cypher_condition = (
            f"EXISTS {{ MATCH (target_au:{admin_unit_type.capitalize()}) "
            f"WHERE toLower(target_au.name) CONTAINS toLower($nl_spatial_target) "
            f"MATCH (s)-[:LOCATED_IN]->(s_loc) "
            f"WHERE (s_loc:{admin_unit_type.capitalize()}) = target_au OR EXISTS {{ MATCH (s_loc:LSOA)-[:LOCATED_IN]->(target_au) }} "
            f"AND EXISTS {{ MATCH (s_loc)-[:DISTANCE_NEAR]->(target_au) }} }}"
            )
            ...
            f"MATCH (s)-[:LOCATED_IN]->(s_loc) "
            f"WHERE (s_loc:{admin_unit_type.capitalize()}) = target_au OR EXISTS {{ MATCH (s_loc:LSOA)-[:LOCATED_IN]->(target_au) }} "
            f"AND EXISTS {{ MATCH (s_loc)-[:INTERSECTS]->(target_au) }} }}"
            )
            ...
            f"MATCH (s)-[:LOCATED_IN]->(s_loc) "
            f"WHERE (s_loc:{admin_unit_type.capitalize()}) = target_au OR EXISTS {{ MATCH (s_loc:LSOA)-[:LOCATED_IN]->(target_au) }} "
            f"AND EXISTS {{ MATCH (s_loc)-[:WITHIN]->(target_au) }} }}"
            )
        else:
            out["unmatched"].append(
