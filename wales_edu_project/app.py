...
def llm_parse_map_question(text: str) -> Dict[str, Any]:
    ...
    if admin_unit_type and spatial_relation and spatial_target:
        cypher_condition = ""
        # All administrative units are labeled 'AdminUnit' in this graph.
        # The specific type (Ward, Community, etc.) is stored in the 'type' property.
        
        type_filter = ""
        if admin_unit_type.lower() in {"ward", "community", "unitary authority", "unitaryauthority"}:
            type_filter = f"AND toLower(target_au.type) CONTAINS '{admin_unit_type.lower()}'"

        if spatial_relation == "near":
            cypher_condition = (
                f"EXISTS {{ MATCH (target_au:AdminUnit) "
                f"WHERE toLower(target_au.name) CONTAINS toLower($nl_spatial_target) {type_filter} "
                f"MATCH (s)-[:LOCATED_IN]->(s_lsoa:LSOA) "
                f"WHERE (s_lsoa)-[:DISTANCE_NEAR]->(target_au) OR (s_lsoa)-[:INTERSECTS]->(target_au) }}"
            )
            ...
            cypher_condition = (
                ...
                f"WHERE toLower(target_au.name) CONTAINS toLower($nl_spatial_target) {type_filter} "
                f"MATCH (s)-[:LOCATED_IN]->(s_lsoa:LSOA) "
                f"WHERE (s_lsoa)-[:INTERSECTS]->(target_au) }}"
            )
            ...
            cypher_condition = (
                ...
                f"WHERE toLower(target_au.name) CONTAINS toLower($nl_spatial_target) {type_filter} "
                f"MATCH (s)-[:LOCATED_IN]->(s_lsoa:LSOA) "
                f"WHERE (s_lsoa)-[:LOCATED_IN]->(target_au) OR (s_lsoa)-[:WITHIN]->(target_au) }}"
            )
        else:
            out["unmatched"].append(
...
def page_map(cfg: Dict[str, str]) -> None:
    ...
    with st.sidebar.expander("Connection Status", expanded=False):
        ...
        st.write(f"**Database:** `{cfg['database']}`")
