...
.geometryorigin {background:#fff3d6; color:#b45309;}
.projectintegrated {background:#eef2ff; color:#3730a3;}
...
def llm_parse_question(
    ...
    try:
        ...
        system = (
            ...
            '"filters": [{"field": "attendance"|"fsm", "op": ">="|"<=", "value": float}] or [], '
...
def render_nl_understanding() -> None:
    ...
    for f in parsed.get("filters") or []:
        chips.append(
            f"<span class='nl-chip nl-chip-focus'>{escape(f.get('field'))} "
            f"{escape(f.get('op'))} {f.get('value')}</span>"
        )
    ...
    for u in parsed["unmatched"]:
        cls = "nl-warn-orange" if "More than one unit matches" in u else "nl-warn"
        warn += f"<div class='{cls}'>{escape(u)}</div>"
...
.nl-warn-orange {{
  margin-top:.5rem; font-size:.82rem; color:{{ev_warn_orange}};
  background:{{ev_warn_orange_bg}}; border-radius:8px; padding:.4rem .6rem;
}}
...
def page_scq_demonstrator(
    ...
    try:
        ...
        # Apply LLM filters if present
        nl_last = st.session_state.get("nl_last")
        if nl_last and nl_last.get("filters") and not result_df.empty:
            for f in nl_last["filters"]:
                field = f.get("field", "").lower()
                op = f.get("op")
                val = f.get("value")
                
                # Map field names to dataframe columns
                col_map = {
                    "attendance": "avg_attendance_pct",
                    "fsm": "avg_fsm_pct",
                    "performance": "avg_capped9",
                    "deprivation": "wimd_rank"
                }
                col = col_map.get(field)
                if col in result_df.columns and op in {">=", "<=", ">", "<"}:
                    try:
                        val = float(val)
                        if op == ">=": result_df = result_df[result_df[col] >= val]
                        elif op == "<=": result_df = result_df[result_df[col] <= val]
                        elif op == ">": result_df = result_df[result_df[col] > val]
                        elif op == "<": result_df = result_df[result_df[col] < val]
                    except Exception:
                        pass
        ...
        strong_answers = {
            "SCQ1": (
                "SCQ1 is answered using the computed <b>LSOA_TOUCHES</b> relation. "
                "It retrieves LSOAs that directly border the selected LSOA and "
                "summarises school indicators. This is a <b>Geometry-origin</b> "
                "capability and does not raise the coverage of the original YAGO2geo model."
            ),
            "SCQ2": (
                "SCQ2 is answered using <b>GRAPH_NEAR</b>. This represents "
                "qualitative proximity derived through graph traversal over the "
                "stored LSOA neighbourhood. It is <b>Project-integrated</b> "
                "capability that adds analytical depth beyond raw geometry."
            ),
            "SCQ3": (
                "SCQ3 implements the cycle-free path definition of between over "
                "<b>LSOA_TOUCHES</b>. It finds regions that lie on a topological "
                "corridor linking two areas. The hop bound is a tractability "
                "necessity for graph traversal."
            ),
            "SCQ4": (
                "SCQ4 is the complement of <b>LSOA_TOUCHES</b>. It returns "
                "regions that share no boundary with the selected LSOA, making "
                "distant policy comparisons visible."
            ),
            "SCQ5": (
                "SCQ5 traverses <b>WITHIN</b> upward through the native "
                "administrative hierarchy. This is a <b>Native</b> YAGO2geo "
                "relation chain."
            ),
            "SCQ6": (
                "SCQ6 traverses <b>WITHIN</b> downward through the native "
                "administrative hierarchy. This is a <b>Native</b> YAGO2geo "
                "relation chain."
            ),
            "SCQ7": (
                "SCQ7 crosses hierarchies using computed <b>INTERSECTS</b>. "
                "It relates administrative units to statistical LSOAs, "
                "answering cross-boundary containment questions."
            ),
            "SCQ8": (
                "SCQ8 combines <b>TOUCHES</b> and <b>INTERSECTS</b> to find "
                "nearby but disjoint administrative units. It represents the "
                "most complex cross-hierarchy traversal in the demonstrator."
            ),
        }

        st.markdown(f"### {t('implemented_answer')}")

        # Provenance and Relation Chain logic
        provenance_info = {
            "SCQ1": ("LSOA --LSOA_TOUCHES--> LSOA", "Geometry-origin"),
            "SCQ2": ("LSOA --GRAPH_NEAR--> LSOA", "Project-integrated"),
            "SCQ3": ("LSOA --LSOA_TOUCHES path--> LSOA", "Geometry-origin"),
            "SCQ4": ("LSOA [NOT LSOA_TOUCHES] LSOA", "Geometry-origin"),
            "SCQ5": ("AdminUnit --WITHIN--> AdminUnit", "Native"),
            "SCQ6": ("AdminUnit --WITHIN--> AdminUnit", "Native"),
            "SCQ7": ("AdminUnit --INTERSECTS--> LSOA", "Geometry-origin"),
            "SCQ8": ("AdminUnit --TOUCHES--> AdminUnit --INTERSECTS--> LSOA", "Project-integrated")
        }
        chain, prov = provenance_info.get(scq_key, ("Unknown", "Unknown"))
        ...
        st.markdown(
            (
                ...
                "<br><br>"
                f"<b>Relation chain:</b> <code>{chain}</code>"
                "<br>"
                f"<b>Provenance:</b> {provenance_badge(prov)}"
                "<br><br>"
                f"<b>{t('eval_status')}:</b> "
