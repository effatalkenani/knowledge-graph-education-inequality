...
NEO4J_USER = "neo4j"
NEO4J_PASSWORD = "UTHT5389utht@"
NEO4J_DATABASE = "wales-education-kg"
...
def get_neo4j_stats():
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        ...
        schools = session.run("MATCH (s:School) RETURN count(s) AS count").single()["count"]
        lsoas = session.run("MATCH (l:LSOA) RETURN count(l) AS count").single()["count"]
    driver.close()
    return nodes, rels, schools, lsoas
...
def preset_to_filters(preset):
    if preset == "Secondary schools in high-deprivation areas near transport":
        return {"school_type": "Secondary", "deprivation": "high_deprivation", "near_transport": True}
    if preset == "Primary schools in high-deprivation areas":
        return {"school_type": "Primary", "deprivation": "high_deprivation"}
    if preset == "All schools far from transport stops":
        return {"near_transport": False}
    if preset == "Secondary schools in medium-deprivation areas":
        return {"school_type": "Secondary", "deprivation": "medium_deprivation"}
    if preset == "Schools in Rhondda Cynon Taf with high deprivation":
        return {"local_authority": "Rhondda", "deprivation": "high_deprivation"}
    if preset == "Schools in Cardiff near transport":
        return {"local_authority": "Cardiff", "near_transport": True}
    return {}
...
def build_map(filtered, graph_nodes, graph_rels):
    if len(filtered) == 0:
        return None
    ...
    legend_html = (
        ...
        f"Nodes: {graph_nodes:,} &nbsp;|&nbsp; Relationships: {graph_rels:,}"
...
def query_neo4j_schools(filters: dict) -> pd.DataFrame:
    conditions = []
    params = {}

    if filters.get("school_type"):
        conditions.append("toLower(s.school_type) CONTAINS toLower($school_type)")
        params["school_type"] = filters["school_type"]

    if filters.get("deprivation"):
        conditions.append("s.deprivation = $deprivation")
        params["deprivation"] = filters["deprivation"]

    if filters.get("near_transport") is not None:
        conditions.append("s.near_transport = $near_transport")
        params["near_transport"] = bool(filters["near_transport"])

    if filters.get("local_authority"):
        conditions.append("toLower(s.local_authority) CONTAINS toLower($local_authority)")
        params["local_authority"] = filters["local_authority"]

    if filters.get("fsm_pct_gt") is not None:
        conditions.append("s.fsm_pct > $fsm_pct_gt")
        params["fsm_pct_gt"] = float(filters["fsm_pct_gt"])

    if filters.get("fsm_pct_lt") is not None:
        conditions.append("s.fsm_pct < $fsm_pct_lt")
        params["fsm_pct_lt"] = float(filters["fsm_pct_lt"])

    if filters.get("attendance_lt") is not None:
        conditions.append("s.attendance_pct < $attendance_lt")
        params["attendance_lt"] = float(filters["attendance_lt"])

    if filters.get("attendance_gt") is not None:
        conditions.append("s.attendance_pct > $attendance_gt")
        params["attendance_gt"] = float(filters["attendance_gt"])

    if filters.get("gcse_lt") is not None:
        conditions.append("s.gcse_pass_pct < $gcse_lt")
        params["gcse_lt"] = float(filters["gcse_lt"])

    if filters.get("gcse_gt") is not None:
        conditions.append("s.gcse_pass_pct > $gcse_gt")
        params["gcse_gt"] = float(filters["gcse_gt"])

    if filters.get("pupils_gt") is not None:
        conditions.append("s.pupils > $pupils_gt")
        params["pupils_gt"] = float(filters["pupils_gt"])
    ...
    RETURN
        ...
        s.local_authority AS local_authority,
        s.latitude AS latitude,
        s.longitude AS longitude,
        s.pupils AS pupils,
        s.near_transport AS near_transport,
        s.deprivation AS deprivation,
        s.wimd_decile AS wimd_decile,
        s.fsm_pct AS fsm_pct,
        s.attendance_pct AS attendance_pct,
        s.gcse_pass_pct AS gcse_pass_pct
    """
...
def get_enriched_schools_from_neo4j():
    return query_neo4j_schools({})

def render_nl_query_tab(enriched, graph_nodes, graph_rels):
    ...
    <p style='margin:0.3rem 0 0;opacity:0.7;font-size:0.82rem;'>
      QPM Parser (LLM or Rule-Based) · Knowledge Graph: Neo4j · QPM Relations: contained_in, near, far_from
    </p>
    ...
    if run_query and user_query.strip():
        ...
        if "error" not in result:
            ...
            st.session_state.nl_matched = query_neo4j_schools(result.get("filters", {}))
    ...
    if st.session_state.nl_result and st.session_state.nl_matched is not None:
        ...
        with tab_map_r:
            m = build_map(matched, graph_nodes, graph_rels)
...
def main():
    ...
    with st.spinner("Loading datasets and building Knowledge Graph..."):
        ...
        enriched = get_enriched_schools_from_neo4j()
        graph_nodes, graph_rels, graph_schools, graph_lsoas = get_neo4j_stats()
    ...
    if preset != "Custom query...":
        ...
        if sel_type != "All":
            sidebar_filters["school_type"] = sel_type

        if sel_dep == "High Deprivation":
            sidebar_filters["deprivation"] = "high_deprivation"
        elif sel_dep == "Medium Deprivation":
            sidebar_filters["deprivation"] = "medium_deprivation"
        elif sel_dep == "Low Deprivation":
            sidebar_filters["deprivation"] = "low_deprivation"

        if sel_transport == "Near transport":
            sidebar_filters["near_transport"] = True
        elif sel_transport == "Far from transport":
            sidebar_filters["near_transport"] = False

        if sel_la != "All":
            sidebar_filters["local_authority"] = sel_la

        filtered = query_neo4j_schools(sidebar_filters)
    ...
    with tab_map:
        if len(filtered) == 0:
            ...
            m = build_map(filtered, graph_nodes, graph_rels)
    ...
    with tab_kg:
        ...
        nodes, rels, schools_count, lsoas_count = get_neo4j_stats()

        with kg_col1:
            st.metric("Total Nodes", f"{nodes:,}")
        with kg_col2:
            st.metric("Total Relationships", f"{rels:,}")
        with kg_col3:
            st.metric("School Nodes", f"{schools_count:,}")
...
- `has_attendance` — school attribute: annual pupil attendance rate
    ...
    # ── NL Query Tab ──────────────────────────────────────────────────────────
    with tab_nl:
        render_nl_query_tab(enriched, graph_nodes, graph_rels)
